...
Try {
	...
	[string]$appVersion = '10.8.2'
	[string]$appArch = 'x64'
	[string]$appLang = 'EN'
	[string]$appRevision = '01'
	[string]$appScriptVersion = '2.0.0'
	[string]$appScriptDate = '2022.06.08'
	[string]$appScriptAuthor = 'KTEUTON'
	...
	$CloseApps = "hh=Help Window,DocDefragmenter=ArcGIS Document Defragmenter,MXDDoctor=MXD Doctor,pythonw=Python Windows,python=Python CMD Prompt,ArcCatalog,ArcGISAdmin=ArcGIS Adminsitrator,ArcGlobe,ArcMap,ArcScene"
	...
	$64bitAppInstallName = "ArcGIS Desktop"
	# Application install Path.
	$64bitAppInstallPath = "$PSScriptRoot\Files\ArcGIS\setup.msi"
	# Application  install parameters.
	$64bitAppInstallParam = "ACCEPTEULA=yes ADDLOCAL=ALL ESRI_LICENSE_HOST=GISAS7 SOFTWARE_CLASS=Viewer SEAT_PREFERENCE=Float DESKTOP_CONFIG=TRUE REBOOT=ReallySuppress /QN" 
	...
	$RemoveAppNamesMSI = @("ArcGIS Desktop", "ArcGIS VBA Compatibility", "Microsoft SQL Server 2012 Native Client")
	#$RemoveAppNamesEXE = @("")
	...
	$PrereqApp3Name = "Microsoft Visual C++ 2015-2019 Redistributable (x86)"
	...
	If ($deploymentType -ine 'Uninstall' -and $deploymentType -ine 'Repair') {
		...
		# Install prerequisite application 2.
		# If installation failed, log results. Exit script. 
		...
		If ($null -eq $RemovePrereqAppNamesCheck2 -And $Null -eq $RemovePrereqAppNamesCheck3) {
			Try {
				Write-log -Message "Installing $PrereqApp2Name."
				Execute-Process -Path "$PrereqApp2InstallPath" -Parameters "$PrereqApp2InstallParam"
				Write-Log -Message "Installing $PrereqApp2Name complete." 
			}
			# If install failed, log results. Exit script.
			Catch [System.Exception] {
				Write-Log -Message "Installing $PrereqApp2Name failed with error: $_."
				Write-Log -Message "Exiting script with error."
				Exit-Script -ExitCode 1627
			}
		}
